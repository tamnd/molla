...
**A first run loads the model twice.** [src/molla/engine/device.mojo](../../src/molla/engine/device.mojo) around lines 486 to 497 runs a complete unbounded host load in order to write the repack sidecar, then reopens the file and loads it again in the same process, and the pages from the first pass are never released. Measured with a hardlinked copy so the cache misses, SmolLM2 under `generate --device=metal` goes from 217 MiB on a hit to 394 MiB on a miss. The 11066 MiB figure that has been quoted for the 8B is that, not a steady state. Fixed by #252, which gives the pages of the first pass back as it goes.

**The repack grows the tensor.** It grew it by 1.38 times when this was written, 6474 MiB against a 4693 MiB GGUF, and it is resident for the life of the process. The layout work of M2c and M2d took that to 4781 MiB, which is 1.9 per cent over the file, and the only forms still paying anything for it are q4_k and q5_k at four bytes a 256 value block. llama.cpp's CPU repack sets `get_alloc_size = nullptr` at `repack.cpp:4828`, so it is byte for byte size neutral, it is never written to disk, there is no `fwrite` or `ofstream` anywhere under `ggml/src/ggml-cpu/`, and the GPU path does not repack at all. That last part is still the real difference and it is what "What this does not answer" in [layout.md](layout.md) is about.
...
And separately from all three, molla's KV cache was f32 at [src/molla/engine/runner.mojo](../../src/molla/engine/runner.mojo) lines 74 to 76 where llama.cpp defaults to f16 and offers q8_0. That was a factor of two on the KV plane for a dtype change, and 3.76 with q8_0. The f16 half of it landed in 0.4.16 and the q8_0 half is the rest of #204.
...
It landed well below on the first attempt. The CUDA tile measured 509 tokens a second on the 8B, about 8.2 TFLOP/s, which is five per cent of the tensor core peak, and it was slower than the ordinary kernel on the two smaller models. So that tile was wrong, which was worth knowing before any assembly got written, and it is exactly what the prediction was there to find out. #212 named what the numbers said was missing: `ld_matrix` for the fragments, a swizzled shared layout, a wider tile and a K loop that stages ahead.

Rebuilt around the first three of those, it measures 1086 tokens a second on the 8B, about 17 TFLOP/s, and it beats the ordinary kernel on every model rather than two of three. That is 1.81 times on the 8B and 1.24 on Qwen, and a tenth of the peak rather than a twentieth. The fourth item, staging the next step while multiplying the current one, is the rest of #212 and is where the next factor is. [prefill.md](prefill.md) has the kernel and what the gate on it is per backend.
