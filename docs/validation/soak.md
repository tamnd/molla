...
The claim is not that memory never grows. A server that has just accepted a thousand connections has not yet grown the buffers those connections need, and the first segments climb on every machine in the fleet, by a factor of three on the smaller ones. The claim is that it stops, so the end of the run is judged against the halfway mark rather than against where it started.

The allowance is a quarter, or four megabytes on machines small enough that a proportion would be a handful of pages. A quarter is wide and it is where two populations of real measurement sit. A reactor's slot table never shrinks, so an hour of reconnecting keeps finding bursts bigger than any it has seen and each new slot costs a read buffer and a write ring, which is step shaped growth that came to eleven percent across the second half on the noisiest machine in the fleet. The leak this soak found was thirty one percent on the same measure on the quietest machine and sixty five percent on the busiest. A future run that lands in between those is a thing to go and find rather than a reason to widen this.

The size of the timing wheels, against the connection tables beside them. A connection has one timer armed at a time and a reactor's slot table never shrinks, so a wheel can never honestly hold more entries than its reactor has slots. That is a gate with no number in it, and it is here because the first four runs turned out to be measuring a wheel that held three and a half million entries against a hundred slots.
