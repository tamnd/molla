...
`scripts/gen-unicode.py` reads the tables out of the Python that is running it and writes `src/molla/text/tables.mojo`. Four of the five come out of `unicodedata` with no network at all. The grapheme break property is not in `unicodedata`, so that one is downloaded from unicode.org at the exact version `unicodedata` reports. Pinning the download to the version already in hand is what keeps the generated file at one Unicode version rather than two, and a version that is not published fails the download rather than producing a mixed table. The current file is Unicode 16.0.0 and holds 3368 category ranges, 393 combining class ranges, 5913 decompositions, 1460 full lowercase mappings and 1481 grapheme break ranges, in 58 KB of source. Everything is a base 64 varint, five payload bits to a digit, and range tables store a delta from the end of the range before rather than an absolute start, which is what keeps a table of 3368 ranges down to a few kilobytes. Hangul is not in the decomposition table at all because 11172 syllables decompose by arithmetic.

There is no way to avoid generating this. The alternative is a dependency on ICU, which is 30 MB of C++ for the five functions a tokenizer needs, and the five functions are a category lookup, a combining class lookup, a decomposition, a lowercase mapping and a grapheme cluster boundary.
...
`src/molla/text/normalize.mojo` is the four normal forms as two operations and two flags. Full decomposition, then a stable sort by combining class, then optionally the composition pass. It also carries the two ways of stripping accents, which are not the same function: `strip_marks` drops non spacing marks and is what the `strip_accents` flag inside a `BertNormalizer` means, and `strip_combining` drops spacing and enclosing marks as well and is what a `StripAccents` normalizer written on its own means. Both spellings are in the tokenizer corpus and running one where the other belongs loses the enclosing keycap off a digit.

`src/molla/text/graphemes.mojo` is UAX #29 extended grapheme clusters, which is the boundary a reader would call a character: a base letter plus whatever marks, joiners and variation selectors hang off it. All of the rules are there including the emoji rule GB11 and the Indic conjunct rule GB9c, and three of them need state carried forward rather than a look at the character before, which is what keeps the walk linear. The only caller so far is the SentencePiece charsmap normalizer, which looks text up one cluster at a time. The break property is a fifth generated table and it is bisected rather than flattened, because about a fifth of tokenizer files ask for it and the rest should not pay for it.
