...
On Metal it is not, and the barrier makes no difference to that, because the problem is not ordering but that the value never leaves the core that wrote it. The store has to be an atomic in every case: an ordinary store followed by an atomic load is still wrong about half the time, and the wrong half is the writes that happened not to have drained yet. Repeated three times on the M4 with the same verdicts and the third column stable to within a few counts.

The middle column is where this probe misleads, and it is worth saying so here rather than only in the issue, because the fused kernel was written against it and had to be rewritten. An atomic store followed by an ordinary load is right every time in the probe and is wrong every time in the fused kernel, deterministically, from the first layer of the first token. The difference is that the probe gives every block an address it has never read before, and the kernel gives a block the same norm scratch at five records of every layer and the same residual stream once a layer for thirty layers. On the second read the line is already in the block's own L1, the barrier does not evict it, and what comes back is a round out of date. The last column is the one to build on: both sides atomic is correct on both devices and correct in the kernel.

So a value that crosses a barrier costs an atomic at both ends. A fused matvec writes one value a row and reads the whole activation vector once for every row, so the store side is one instruction a row and the load side is one for every column of every row, in the inner loop. It lands on the activation vector rather than on the weights, which is the small hot operand rather than the one the matvec is bandwidth bound by, and that is the reason to expect it to be affordable rather than a measurement saying it is.
...
What the Metal atomics cost. The store is one instruction on a value the kernel had in a register anyway and should be free. The load is one for every column of every row of every matvec, which is the thing to be worried about, and the reason to expect it to survive is that it lands on the activation vector rather than on the weights. Neither is measured. It is the first thing the fused kernel should report.
