...
Two independent measurements agree with this reading. The first is that the 8B, which is more than ten times the size of anything else in the tables, has the smallest gap of the three at 2.4 times against 3.1 and 3.3: a fixed cost per launch is a smaller share of a launch that does more work, whereas a slow kernel would stay slow in proportion. The two small models do not rank cleanly against each other, because one is Q8_0 with thirty layers and the other is q4_K_M with twenty four, so the reading rests on the 8B rather than on a trend across all three. The second is the roofline. 138 MiB of weights at the 4090's 1008 GB/s is 0.144 ms per token, so the hardware ceiling for SmolLM2 decode is about 7000 tokens per second. molla is at 254.5 and llama.cpp is at 782.5, which is to say both are far from the card's limit and llama.cpp is only better at the same thing molla is bad at.
