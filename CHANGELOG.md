...
The fifth and last of the batching stages, and the measurement that says it was aimed at the wrong thing. Fair mode does not move the server numbers on any workload measured here. Three runs of each mode at sixteen, thirty two and sixty four streams differ by less than the run to run spread, and so does the churn case that frees and takes slots all the way through. The arithmetic says why: sixty four short prompts are three steps of prefill and about a seventh of a second, against a slowest first token of about 1.9 seconds, so the cost is not in the prefill chunks and no ordering of them reaches it. It is not the client either, since sixty four separate curl processes give the same numbers as the Python client to within a few per cent. There was a second or more of serialized per request work between the socket and the first step and it was not the batch. It is fixed below, and docs/validation/batching.md carries the tables and the reasoning for both halves.

### Fixed

- A worker no longer generates a stream's first token in the same turn it admitted it. A step carries every stream in the batch, so being in the batch when the step runs is nearly free and being the reason a step runs is the whole cost, and the worker was paying it once per request in a row: it read one request off one readable socket, admitted it, ran a whole forward pass for that one stream, and only then looked at the second socket. Sixty four requests arriving together cost sixty four sequential forward passes before the last one was in the batch. On the 4090 with Qwen 2.5 0.5B Q4_K_M and sixty four streams, the slowest first token falls from 2028 ms to 386 ms and the longest a stream ever waits between two of its own tokens falls from 1789 ms to 109 ms, with aggregate throughput up from 1259 to 1320 tokens a second. At sixteen streams it is 312 ms to 113 ms and 227 ms to 40 ms. The median and the ninety fifth percentile barely move, because this was never a throughput problem and a percentile across streams cannot see a stall that lands on a different stream every step. Reported as #292.
- `MOLLA_TRACE=1` prints one line per request saying when it was accepted, how long until it was read, encoded and admitted, and how long after that its first token went out. Off by default and read once at startup. It is what found the above, and the reason it exists is that every one of those numbers was small and the sum of them was not the second the client saw.
...
- A server with every slot busy answers 503 and says that retrying is the right thing, rather than treating a request it could serve in a moment as a request it cannot serve. A request longer than the whole context is still a 400, because retrying that one is not going to help.

### Changed

- `Runner` holds one job per slot rather than one request in flight, so the calls that used to be about the request now take which job they are about. A connection keeps its job handle from the first token to the last and hands it back when it closes, and the state a completion streams from moved to the connection with it, which is what lets two connections on one worker each be halfway through an answer.
...
- `DeviceBatch` steps several streams over one pool. Each step is filled from every stream that has work until the cap is full: a stream still reading its prompt gives as much of it as there is room for, a stream generating gives the one token it last produced. Chunked prefill needed no mechanism of its own as a result, since the cap the batch has anyway is what cuts a long prompt up and the streams that are decoding decode in between the pieces. A stream that has not finished its prompt gets no answer, which is what makes the rows a step asks for the streams that reached the end of what they owed. That is the second half of the fourth of the five stages in [docs/validation/batching.md](docs/validation/batching.md).
- `molla batch` runs several copies of a prompt at once and reports aggregate tokens a second, time to first token per stream, and the spread of inter token latency, which are the three numbers a single stream run has no version of. Every stream gets the same prompt and greedy settings, so every stream has to write the same tokens, and the command checks that rather than leaving it to whoever reads the text. Two sequences whose cells got mixed up would disagree there.
...
- `Sampler.pick` takes which row of the logits to read, for a caller holding the answers a batch brought back. A single sequence reads row zero, which is where it always read.
...
- A fuzz over pool layouts. Each trial takes a decoy of a random length, gives a random half of it back so the pool has holes in it rather than an offset, cuts the same prompt into chunks of a random length, and runs those chunks twice, once into the holed pool and once into an empty one. Nothing about the first run is contiguous and everything about the second is, and the logits have to agree. Eight trials from a fixed seed, so a failure is reachable a second time. This is the gate #31 asks for as far as it reaches while there is one sequence, and the version over conversation trees needs the second sequence that #32 brings.
...
- What that mask costs, measured on the 4090 and written into [docs/validation/paging.md](docs/validation/paging.md). Eight per cent of decode attention, flat from 64 keys to 2048, over a pool where every cell is live so that both sides read the same bytes and run the same grid. It is small because the kernel was already reading 256 bytes of key row for every entry it masks, so one more four byte load is three per cent of the traffic and the rest is a compare. `scripts/attend_probe.mojo` grew the fourth table that says so, and was repaired on the way: it has passed `DeviceVec` keys since the cache went to float16, and nothing builds the probes, so it had not compiled for several releases.
- The attention kernels take the same window the host one does, which finishes the third of the four stages in [docs/validation/paging.md](docs/validation/paging.md). `device_attend_paged` is handed a pool, a `DeviceInts` of one position a cell and the host copy of it, and it scans cells rather than a run of keys, so the count no longer grows with the token in a chunk and causality is a comparison against the query's position instead of where the loop stops. The single kernel and the split one share the mask through one function, which is the two halves of `molla.nn.attention` term for term, and the branch is on a kernel argument so every thread of every block takes the same side of it. The unpaged path is untouched and still derives causality from the count. What the split path gains is a slice that holds none of this sequence's cells, which a contiguous window produced only at the front of a long context and a pool produces anywhere, and it comes back as nothing seen rather than as a division by zero. The test scatters positions over a pool with a stride of seven and fills the cells between them with junk that is free in half the cases and holds a future position in the other half, which are the two reasons a cell can be invisible.
- Attention can be masked by the position each cell holds rather than by how many keys it was handed, which is the host half of the third of the four stages in [docs/validation/paging.md](docs/validation/paging.md). `attend` takes a window, one entry a cell, holding the position that cell holds for this sequence or a negative for one it may not read, and `AttnSpec.sees` answers over a cell what `visible` answers over an offset into a run. Causality, ownership, the sliding window and the sinks all come out of that one entry, so the mask does not grow with the batch: a prefill chunk of 256 tokens against 4096 cells is 16 KiB of positions where llama.cpp's matrix of floats would be a megabyte. The window is built by `CellTable.held` over `CellTable.window`, which is the frontier rounded up to a pad so the launch shape stops changing every token. An empty window means the rows are a run starting at zero, which is what every caller passes today, so there is one loop rather than a paged path and an unpaged one. The test is the numbers from the contiguous case, to the last digit, with the same four keys scattered over six cells out of order.
- A scattered cache store, which is the other half of that stage and the only place paging reaches a kernel. `DeviceInts` holds the vector of cell indices for a step, one entry a token, and `device_store_kv_at` writes row `t` of a finished key or value projection into cell `cells[t]` of the pool. A sequence whose cells are not contiguous now writes in one launch rather than one launch a token, and a sequence whose cells happen to be contiguous writes exactly what the contiguous store writes, bit for bit, which is what the test compares. The contiguous store is untouched, since it is a load and a convert with no arithmetic in it and every decode takes it today. Nothing calls the scattered one yet, because the reader that makes scattered cells correct is the mask in the stage after this.
- A cell table in `molla.engine.cache`, which is the host side bookkeeping for the second of the four stages in [docs/validation/paging.md](docs/validation/paging.md). It is two vectors over the pool, the position each cell holds and a word of owner bits, plus a search hint. A cell is one token, so there is no block size to pick, no partial block to copy when two sequences diverge, and no table for a kernel to walk. Sharing a prefix sets a second bit on the cells that already hold it and copies nothing, and a sequence leaving frees only the cells no other sequence still claims. One owner word caps a pool at sixty four sequences in flight against M3's gate of sixteen, which is eight bytes a cell where llama.cpp's bitset of 256 is thirty two. Nothing calls it yet. It lands on its own because a free list and an owner set can be tested to exhaustion without a card, and the stage after it wants to be a change to attention alone.
