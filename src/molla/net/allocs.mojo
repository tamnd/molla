...
def _drain_until_closed(fd: Int) -> Int:
    ...
    Every request this command sends asks for `Connection: close` on the last
    one, so the end of the answer is the end of the socket and there is no
    framing to parse on the client side.
    """
...
def _load() -> String:
    ...
    Every kind of request molla can currently answer, in one string, ending
    with the one that asks the server to close. Building it once and sending it
    whole means the client is not the thing being measured.
    """
    var out = String("")
    out += "GET / HTTP/1.1\r\nHost: molla\r\n\r\n"
    out += "HEAD / HTTP/1.1\r\nHost: molla\r\n\r\n"
...
def _pass(
    ...
    Every connection in a round is opened before any of them is written to, and
    all of them are written to before any of them is read. That ordering is the
    difference between exercising the reactor's whole table and exercising one
    slot. Opening and finishing one connection at a time lets the server accept,
    answer and free the same slot every time, which proves that one slot is
    reused and nothing about the other sixty three.
