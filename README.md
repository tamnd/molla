...
D6 says the engine is ours, and the reversal condition on it has a number in it, so molla is measured against llama.cpp and Ollama on the same file, the same prompt and the same card, one table per machine, in [docs/validation/bench.md](docs/validation/bench.md). Decode on CUDA is currently 2.2 times off llama.cpp and prefill is eighty times off, because molla has no batched prefill yet.
...
That is the one that is a server. It answers `/v1/chat/completions`, `/v1/completions` and `/v1/models`, streaming or not, and takes `--host`, `--port`, `--ctx` and `--device`. It runs on an accelerator when there is one the model fits on, and `--device=cpu` keeps it on the host, which is written up in [docs/validation/backend.md](docs/validation/backend.md). It runs one sequence at a time and refuses a second with a 503 rather than queueing it, and a field it does not implement is a 400 that names the field rather than a field quietly ignored. See [docs/validation/openai.md](docs/validation/openai.md).
