...
> **Status: early.** `molla serve` answers OpenAI chat and completion requests against a GGUF file on disk, streaming or not, and the OpenAI Python SDK works against it by changing `base_url`. One sequence at a time, no scheduler, no batching, and about ten tokens a second on a small model, which is what the rest of M2 and M3 are for. Everything else on the list below is still a plan.
...
| Config, structured logging, counters, and the admin routes | nothing tells an operator what a running server is doing, done, see [docs/validation/ops.md](docs/validation/ops.md) |
...
| A steady state request that allocates nothing, checked on every commit | a claim like that stops being true within a month unless something checks it, done, see [docs/validation/allocations.md](docs/validation/allocations.md) |
| A server that holds a thousand mixed connections for an hour without drifting | correctness under load is not the same claim as correctness, done, nightly on both platforms, see [docs/validation/soak.md](docs/validation/soak.md) |
...
| A Jinja2 subset for chat templates | no template engine, done, same bytes as Python over 494 real chat templates and 9500 renders on every commit, every excluded construct refused at model load time, see [docs/validation/jinja.md](docs/validation/jinja.md) |
| BPE, Unigram, and WordPiece tokenizers | no tokenizer, all three done from `tokenizer.json`, same ids as Hugging Face `tokenizers` over 4560 differential cases and eight million bytes on four models, and over a corpus of 338 real files with nothing refused and nothing mismatched, see [docs/validation/tokenizer.md](docs/validation/tokenizer.md) and [docs/validation/text.md](docs/validation/text.md) |
...
That builds and runs today on macOS arm64, Linux x86_64, and Linux arm64.

```console
./build/molla serve ~/models/SmolLM2-135M-Instruct-Q8_0.gguf ~/models/tok/smollm2.json
export OPENAI_BASE_URL=http://127.0.0.1:8000/v1
```

That is the one that is a server. It answers `/v1/chat/completions`, `/v1/completions` and `/v1/models`, streaming or not, and takes `--host`, `--port`, `--ctx`, `--device` and `--cache-type`. It runs on an accelerator when there is one the model fits on, and `--device=cpu` keeps it on the host, which is written up in [docs/validation/backend.md](docs/validation/backend.md). `--cache-type=q8_0` holds the keys and values at a byte a value plus a factor a block of thirty two rather than at float16, which is a little over half the memory the cache would otherwise take and is the setting to reach for when a long conversation is what does not fit. `generate` takes it too. See [docs/validation/kvcache.md](docs/validation/kvcache.md). It runs one sequence at a time and refuses a second with a 503 rather than queueing it, and a field it does not implement is a 400 that names the field rather than a field quietly ignored. See [docs/validation/openai.md](docs/validation/openai.md).

The rest are spikes and tools rather than features. `molla version` prints the toolchain and what it detected about your machine. `molla echo` runs the M0 socket spike, which is a TCP echo server, `molla http` runs the M0 throughput spike, which answers every request with the same fixed body, `molla gguf <path>` prints the metadata and tensor directory of a model file, `molla safetensors <path>` does the same for a safetensors file or a Hugging Face directory, `molla spec <path>` prints what that model is and what this build could do with it, whichever of the two formats it arrived in, `molla devices` lists what this machine can put a tensor on, `molla tokenize <tokenizer.json> "<prompt>"` prints how many tokens a prompt is without opening a model, `molla load <path>` reads a model's weights onto whichever of those it picked and reports each stage as it goes, `molla generate <model.gguf> <tokenizer.json> "<prompt>"` prefills that prompt and decodes it, printing tokens as they arrive, greedily unless you pass `--temp` and the rest of the sampling flags, and on a card unless you pass `--device=cpu`, `molla tls <host>` connects over HTTPS and prints what was negotiated, `molla pull <ref>` fetches a blob from ghcr.io and checks its digest, `molla drain` loads a server up, sends itself SIGTERM, and checks every client got every answer, `molla allocs` runs a mixed load twice and fails if the second pass allocated anything at all, and `molla httpsoak` holds a thousand connections of five different kinds of traffic for an hour and checks that nothing leaked, queued or slowed down. None of them is a molla feature. See [docs/validation/toolchain.md](docs/validation/toolchain.md) for the pinned version, the machines it has actually been run on, and the notes on what Mojo 1.0 turned out to look like in practice.
